...
**Phase 1 (auth and notes) — mostly done.** Sign up, sign in, sign out, protected routes (`/dashboard`, `/notes/[id]`, `/review`) via `src/proxy.ts`. A dashboard for creating subjects and notes. A markdown note editor with debounced autosave. Outstanding: password reset and Google OAuth sign-in (see CLAUDE.md section 9).
