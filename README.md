...
CI runs lint, typecheck, unit tests, build, and Playwright end to end tests on every push via `.github/workflows/ci.yml`.
...
**Phase 4 (discovery and paths): done.** LLM-interpreted subject search on the public home page, usable signed out (`/study/[query]`). Serper for web results, YouTube Data API for videos and playlists. Real video/playlist ids are extracted in code (`src/lib/youtube.ts`) rather than left for the model to guess, sorted newest-upload-first for every subject so a search always surfaces current information. A query naming a certification whose official name uses "+" (Network+, Security+, A+...) is normalized before it reaches either search API, so typing "Network Plus" still finds the "Network+" material instead of returning weaker results for a phrase nobody actually published under (`normalizeCertPlus` in `src/lib/certQuery.ts`). Every candidate resource is checked for a live response (`src/lib/link-checker.ts`) before a path can be saved; broken links are discarded, not shown. Free resources are sorted ahead of paid in code, not left to the model's word. A query naming a certification or exam (a CompTIA exam, AWS Solutions Architect, and so on) gets a staged, multi-part path instead of one flat list, free-first within every stage. Saving a path or acting on it (`src/app/dashboard/actions.ts`) prompts sign-in for signed-out users. `/study/[query]` is cached with a 7-day ISR revalidation (`export const revalidate` in `page.tsx`) - it wasn't taking effect before this: the page's data came from a Server Action that also touched a cookie-reading Supabase client for unrelated SEO bookkeeping, and any `cookies()` read in a route's render path forces the whole route dynamic regardless of a `revalidate` export. That bookkeeping now uses a cookie-free client (`src/utils/supabase/public.ts`). Measured effect: the full Serper + YouTube + Gemini + link-check pipeline ran on every single visit before this (21.5s in production); now it runs once per revalidation window.

**Phase 5 (polish): mostly done.** A landing page with a real interactive markdown-to-recall-card demo (not a screenshot of one), an embedded video split-view workspace at the spec'd 50-60% desktop width with timestamp capture and playback position that resumes after a refresh or sign-out (auto-fetched transcripts were tried and removed for being unreliable on Netlify's IPs, see `docs/decisions/0003` and `0004`), a contrast pass, SEO metadata (sitemap, robots, Open Graph/Twitter tags), and a copy pass to keep marketing and UI text in the site's own voice (see section 19). A cursor-tracked accent spotlight on hover (`TiltCard`, applied everywhere it's used) and a count-up entrance on the dashboard's stat numbers, both skipped entirely under `prefers-reduced-motion` per section 13. A lightweight markdown formatting toolbar (bold/italic/heading/list, insert-at-cursor) above the note editor — still a plain textarea underneath, not a rich text engine, per section 20. "Take notes" on a path's video step now works when that step is a full playlist (resolves the playlist's first video server-side), not just a single video. A mobile-responsive pass on the dashboard's path tracker (wrapping/overlap fix, 44px touch targets). The account settings page (`/settings`) got a restyle: a status card showing real "studying since" account age instead of a placeholder, and a native `<details>` disclosure for the change-password form instead of it always being expanded; the password reset page (`/auth/reset-password`) gained a "Cancel" link back to sign-in. Both password forms now use a short placeholder instead of showing the full character-requirement text before there's anything to correct (it still appears as the actual validation error). The note editor's formatting toolbar buttons got real styled hover tooltips (a CSS-only `group-hover` pattern, matching `dashboard/SubjectNav.tsx`) in place of the native browser `title`, plus Ctrl/Cmd+B and Ctrl/Cmd+I shortcuts alongside the existing Ctrl/Cmd+K; a one-line tip above the editor reminds how to make a card and points at the Editor Guide. Manual save (a Ctrl/Cmd+S shortcut, and the save-status pill in the header is now itself a save button) runs alongside the existing debounced autosave rather than replacing it. The video split-view's paused-state thumbnail now uses `object-contain` instead of `object-cover` - the facade's container runs far taller than the thumbnail's native 16:9, and `cover` was cropping the video's own title text off both edges to fill it. The dashboard header is two independent layouts below/above the `sm` breakpoint rather than one reflowed with CSS order: on mobile the logo and account menu share the first row (top right) with the subject switcher on its own row below, matching how the account menu's own trigger drops its name text to an icon-only button below `sm` (an explicit `aria-label` keeps it readable to screen readers once the visible text is hidden); on desktop it's the original single row. A "Welcome back" line (real account name, never a placeholder) sits above the hero banner's badge. Switching subjects (`SubjectSwitcher`) now calls the server action directly and follows it with an explicit `router.refresh()` instead of relying on a plain form's implicit post-action refresh, which wasn't reliably picking up a change to the cookie the whole dashboard tree is keyed off. The landing page's interactive flashcard demo got a fourth card (Organic Chemistry) and now actually centers on mobile - the flex container was missing `items-center`, so on the single-column mobile layout each card rendered flush-left instead. Playwright (`tests/discovery.spec.ts`, Phase 4's end to end spec) now actually runs: wired into `npm run test:e2e`, the deploy checklist, and CI (which previously stopped at `npm run build` and never executed it). Running it for the first time surfaced a stale assertion in that same spec (checking for page text that no longer existed) - fixed alongside the wiring.
