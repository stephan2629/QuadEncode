...
export default function StudyLoading() {
  return (
    <div role="status" aria-live="polite" aria-busy="true" className="min-h-dvh bg-[#0a0908] text-white flex flex-col items-center justify-center p-6 text-center">
      <div className="relative mb-8">
        <div className="absolute inset-0 bg-accent/20 blur-xl rounded-full animate-pulse motion-reduce:animate-none" />
        <Loader2 className="w-12 h-12 text-accent animate-spin motion-reduce:animate-none relative z-10" aria-hidden="true" />
      </div>
      <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-accent/10 border border-accent/30 text-accent text-xs font-mono font-bold uppercase tracking-wider mb-4">
        <Sparkles className="w-3.5 h-3.5 animate-pulse motion-reduce:animate-none" aria-hidden="true" /> Curating Learning Path
      </div>
